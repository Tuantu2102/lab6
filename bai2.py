...
class AntiVirusGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        for i, button_text in enumerate(buttons):
            ...
            if i < 2:
                buttons_layout1.addWidget(button)
            elif i < 4:
                buttons_layout2.addWidget(button)
            else:
                # Add the last button to a new layout for the third row
                buttons_layout3 = QHBoxLayout()
                buttons_layout3.addWidget(button)
        
        content_layout.addLayout(buttons_layout1)
        content_layout.addLayout(buttons_layout2)
        if 'buttons_layout3' in locals():
            content_layout.addLayout(buttons_layout3)
        content_layout.addStretch()

        footer = QLabel("Get Premium to Enable: (Web Protection), (Full Scan), (Simple Update)")
        footer.setAlignment(Qt.AlignCenter)  # Center the footer
